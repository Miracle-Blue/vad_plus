...
class VADHandleInternal {
    ...
    deinit {
        ...
        invalidateCallback()
        stopListening()
        // Clean up cached audio components
        audioEngine = nil
        audioConverter = nil
        outputFormat = nil
    ...
    func initialize(config: VADConfigInternal, modelPath: String?) throws {
        ...
        // Pre-configure audio session and engine for faster startup
        try preconfigureAudio()
        
        sendEvent(type: .initialized)
    }
    
    /// Pre-configures audio session and engine during initialization for faster start
    private func preconfigureAudio() throws {
        #if os(iOS)
        if !isAudioSessionConfigured {
            ...
            try audioSession.setPreferredIOBufferDuration(0.005) // 5ms instead of 20ms
            try audioSession.setActive(true, options: .notifyOthersOnDeactivation)
            isAudioSessionConfigured = true
            
            if config.isDebug {
                print("VadPlus: Audio session pre-configured")
            }
        }
        #endif
        
        // Pre-create audio engine and format
        if audioEngine == nil {
            ...
            if let outFmt = outputFormat,
               inputFormat.sampleRate != outFmt.sampleRate || inputFormat.channelCount != 1 {
                audioConverter = AVAudioConverter(from: inputFormat, to: outFmt)
            }
            
            // Pre-prepare the engine (this is expensive)
            engine.prepare()
            
            if config.isDebug {
                print("VadPlus: Audio engine pre-prepared")
            }
        }
    }
    ...
    private func findBundledModel() -> String? {
        ...
        for name in modelNames {
            if let path = Bundle.main.path(forResource: name, ofType: "onnx") {
                if config.isDebug {
                    print("VadPlus: Found model in main bundle: \(path)")
                }
                return path
            }
        }
        
        // Try vad_plus_assets resource bundle (CocoaPods resource_bundles)
        let classBundle = Bundle(for: type(of: self))
        if let resourceBundleURL = classBundle.url(forResource: "vad_plus_assets", withExtension: "bundle"),
           let resourceBundle = Bundle(url: resourceBundleURL) {
            for name in modelNames {
                if let path = resourceBundle.path(forResource: name, ofType: "onnx") {
                    if config.isDebug {
                        print("VadPlus: Found model in vad_plus_assets bundle: \(path)")
                    }
                    return path
                }
            }
        ...
        // Try to find resource bundle in main bundle
        if let resourceBundleURL = Bundle.main.url(forResource: "vad_plus_assets", withExtension: "bundle"),
           let resourceBundle = Bundle(url: resourceBundleURL) {
            for name in modelNames {
                if let path = resourceBundle.path(forResource: name, ofType: "onnx") {
                    if config.isDebug {
                        print("VadPlus: Found model in main's vad_plus_assets bundle: \(path)")
                    }
                    return path
                }
            }
        ...
        // Try plugin bundle (for CocoaPods)
        ...
        for bundle in pluginBundles {
            for name in modelNames {
                if let path = bundle.path(forResource: name, ofType: "onnx") {
                    if config.isDebug {
                        print("VadPlus: Found model in plugin bundle: \(path)")
                    }
                    return path
                }
            }
            // Also check for resource bundle inside plugin bundle
            if let resourceBundleURL = bundle.url(forResource: "vad_plus_assets", withExtension: "bundle"),
               let resourceBundle = Bundle(url: resourceBundleURL) {
                for name in modelNames {
                    if let path = resourceBundle.path(forResource: name, ofType: "onnx") {
                        if config.isDebug {
                            print("VadPlus: Found model in plugin's resource bundle: \(path)")
                        }
                        return path
                    }
                }
        ...
        // Try to find bundle by class (works better with CocoaPods)
        for name in modelNames {
            if let path = classBundle.path(forResource: name, ofType: "onnx") {
                if config.isDebug {
                    print("VadPlus: Found model in class bundle: \(path)")
                }
                return path
            }
        }
        ...
        for frameworkPath in frameworkPaths {
            for name in modelNames {
                ...
                if FileManager.default.fileExists(atPath: path) {
                    if config.isDebug {
                        print("VadPlus: Found model at framework path: \(path)")
                    }
                    return path
                }
            }
        ...
        // Debug: Print all searched locations
        if config.isDebug {
            print("VadPlus: Model not found. Main bundle path: \(Bundle.main.bundlePath)")
            print("VadPlus: Class bundle path: \(classBundle.bundlePath)")
            print("VadPlus: All bundles: \(Bundle.allBundles.map { $0.bundlePath })")
        }
    ...
    func startListening() throws {
        ...
        // Ensure audio is pre-configured (if not already done in init)
        #if os(iOS)
        if !isAudioSessionConfigured {
            let audioSession = AVAudioSession.sharedInstance()
            try audioSession.setCategory(
        ...
        // Create engine if not pre-created
        if audioEngine == nil {
            audioEngine = AVAudioEngine()
        }
        ...
        // Create output format if not pre-created
        if outputFormat == nil {
            outputFormat = AVAudioFormat(
                commonFormat: .pcmFormatFloat32,
                sampleRate: Double(config.sampleRate),
        ...
        // Create converter if not pre-created and needed
        if audioConverter == nil && (inputFormat.sampleRate != outFmt.sampleRate || inputFormat.channelCount != 1) {
            audioConverter = AVAudioConverter(from: inputFormat, to: outFmt)
        }
        ...
        inputNode.installTap(onBus: 0, bufferSize: bufferSize, format: inputFormat) { [weak self] buffer, _ in
            ...
            if let converter = converter {
                guard let convertedBuffer = AVAudioPCMBuffer(
                    pcmFormat: outFmt,
                    frameCapacity: AVAudioFrameCount(Double(buffer.frameLength) * outFmt.sampleRate / inputFormat.sampleRate)
                ) else { return }
    ...
    func stopListening() {
        ...
        audioEngine?.stop()
        // Keep audioEngine, audioConverter, outputFormat for faster restart
        // They will be recreated if needed in startListening
        
        resetStates()
        
        // NOTE: We intentionally do NOT call setActive(false) here because it would
        // interrupt other audio that's playing (like music). The audio session will
        // be reconfigured when startListening() is called again, and iOS will
        // manage the session lifecycle appropriately.
        // Also keep isAudioSessionConfigured = true for faster restart
